...
class VectorQuantizer2(nn.Module):
    ...
    def viz_from_ms_h_BChw(self, ms_h_BChw: List[torch.Tensor], same_shape=True, last_one=False) -> Union[List[torch.Tensor], torch.Tensor]:
        ls_f_hat_BChw = []
    ...
    def infer_multiscale_idx_or_accu(self, f_BChw: torch.Tensor, return_accu: bool, v_patch_nums: Optional[Sequence[Union[int, Tuple[int, int]]]] = None) -> List[torch.Tensor]:  # z_BChw is the feature from inp_img_no_grad
        B, C, H, W = f_BChw.shape
        f_no_grad = f_BChw.detach()
        f_rest = f_no_grad.clone()
        f_hat = torch.zeros_like(f_rest)
        ...
        for si, (ph, pw) in enumerate(patch_hws): # from small to large
            ...
            if return_accu:
                ret.append(f_hat.clone())           # used in `get_recon_for_tb_lg` when training VAE
            else:
                ret.append(idx_N.reshape(B, ph*pw))     # used in `get_GPT_ground_truth` when training VAR, or inference VAE
        
        return ret
    
    def fuse_multiscale_idx_as_gpt_inp_BL(self, gt_ms_idx_Bl: List[torch.Tensor]) -> torch.Tensor:   # only used in VAR teacher forcing
        gpt_inp = []
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            for si in range(SN-1):
                ...
                pn = self.v_patch_nums[si+1]
                gpt_inp.append(F.interpolate(f_hat, size=(pn, pn), mode='area').view(B, C, -1).transpose(1, 2))
        
        return torch.cat(gpt_inp, dim=1) if len(gpt_inp) else None    # cat BlCs to BLC, this should be float32
